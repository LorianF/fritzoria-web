...
before(async()=>{
 server=spawn(process.execPath,['--import','./tests/simulation-server-fixture.mjs','node_modules/next/dist/bin/next','start','--hostname','127.0.0.1','--port','5177'],{env:{...process.env,VERCEL_ENV:'preview',XENDIT_SECRET_KEY:'xnd_development_fixture',SUPABASE_SERVICE_ROLE_KEY:'fixture-service',XENDIT_WEBHOOK_TOKEN:'fixture-webhook'},stdio:['ignore','pipe','pipe'],windowsHide:true});
...
test('HTTP checkout sandbox: guards, server quote, one session per retry and verified history',async()=>{
 ...
 const first=await post(body,'customer');assert.equal(first.status,201,await first.clone().text());
 ...
 const webhook=(token,payload)=>fetch(base+'/api/payments/webhook',{method:'POST',headers:{'x-callback-token':token,'Content-Type':'application/json'},body:JSON.stringify(payload)});
 const event={event:'payment_session.completed',data:{payment_session_id:result.payment.id,status:'COMPLETED',amount:1}};
 assert.equal((await webhook('wrong',event)).status,401);
 assert.equal((await webhook('fixture-webhook',event)).status,200);
 assert.equal((await webhook('fixture-webhook',event)).status,200);
 assert.equal((await webhook('fixture-webhook',{event:'payment_session.completed',data:{payment_session_id:'bad'}})).status,400);
});
