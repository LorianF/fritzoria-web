import { testAdminContext, testKey, testPayload, publicTestSession, xenditRequest, PaymentTestError, TEST_CHANNELS } from "@/lib/payments/xendit-test";
...
type Order = { id: string; total: number; channel: string; session_id: string | null };
async function paymentFor(order: Order, userId: string, key: string) {
  if (!order.session_id) return null;
  const data = await xenditRequest(key, `/sessions/${encodeURIComponent(order.session_id)}`);
  if (data.metadata?.fritzoria_order_id !== order.id) throw new PaymentTestError("Sesi tidak cocok dengan pesanan simulasi.", 502);
  const payment = publicTestSession(data, userId, order.total);
  if (payment.channel !== order.channel) throw new PaymentTestError("Channel pesanan simulasi tidak sesuai.", 502);
  return payment;
}
...
export async function GET(request: Request) {
  try {
    const key = testKey();
    const {db,userId} = await testAdminContext(request);
    ...
    if (id) {
      ...
      return Response.json({order:data,payment:await paymentFor(data,userId,key)}, {headers});
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (existing.data) {
      ...
      return Response.json({order:existing.data,payment:await paymentFor(existing.data,userId,key)}, {headers});
    }
    ...
    const row={id:body.key,user_id:userId,channel:body.channel,courier:body.courier,...quote};
    // Reserve the unique attempt BEFORE contacting Xendit. Concurrent/replayed requests cannot create two sessions.
    const inserted = await db.from("simulation_orders").insert(row).select("*").single();
    if (inserted.error) throw new PaymentTestError("Percobaan sudah berjalan atau gagal disimpan. Periksa Riwayat Mode Tes sebelum mencoba lagi.",409);
    const payload={...testPayload(userId,body.channel,`FRTEST-${body.key}`),amount:quote.total,
      description:`MODE TES Fritzoria ${body.key} — simulasi buku, tidak dikirim`,
    ...
    try {
      ...
      const saved=await db.from("simulation_orders").update({session_id:payment.id}).eq("id",body.key).eq("user_id",userId).select("*").single();
      if (saved.error) throw new PaymentTestError(`Sesi Xendit dibuat (${payment.id}), tetapi penyimpanan belum terkonfirmasi. Jangan membuat ulang; simpan ID sesi untuk rekonsiliasi.`,503);
      return Response.json({order:saved.data,payment}, {status:201,headers});
    } catch(error) {
      ...
      await db.from("simulation_orders").update({setup_error:message}).eq("id",body.key).eq("user_id",userId);
      throw error;
    }
  } catch(error) { return fail(error); }
}
